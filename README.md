...
Frogbot was never actually coupled to running inside the repository it scans. It
is coupled to environment variables that CI providers happen to auto-fill, and
every one of them is explicitly overridable:
...
| The source code | Frogbot clones/downloads it itself — `actions/checkout` is not used |
...
`GITHUB_EVENT_PATH` or `GITHUB_REPOSITORY` anywhere in the Frogbot codebase.
...
`JF_GIT_TOKEN` is the one thing that has no free equivalent here. A workflow's
...
single target repo — not a stored credential. See "known gaps" below.
...
3. **No merge gate.** Making this block merges needs an organization ruleset
   requiring a status check from the JFrog App — which is a separate mechanism
   from these workflows, and is the next thing to prove.
4. **`scan-repository` also opens fix PRs.** It is not a scan-only path, so it
   runs package managers and therefore does depend on the build environment.
   Splitting scan from fix is a prerequisite for a purely centralized detective
   capability.
